""" This is a support file for strategy.py"""
...
def record_trades(stock_ticker, quantity, side, trade_type, now, limit_price, stock_sentiment, txt_file="./trades.txt",
                  sqlite_db="trades.db"):
    """
    ...
    limit_price: Price in which to execute the trade
    stock_sentiment: News sentiment at time of trade execution
    ...
    init_query = "CREATE TABLE IF NOT EXISTS record_trades(stock_ticker TEXT, quantity_traded INT, side TEXT, trade_type TEXT, date TEXT, price TEXT, stock_sentiment TEXT)"

    query = "INSERT INTO record_trades (stock_ticker, quantity_traded, side, trade_type, date, price, stock_sentiment) VALUES" \
            " (?, ?, ?, ?, ?, ?, ?)"

    with con:
        cur = con.cursor()
        cur.execute(init_query)
        cur.execute(query, (stock_ticker, quantity, side, trade_type, now, limit_price, stock_sentiment))

    # Save query in .txt file as well
    with open(f"{txt_file}", 'a') as file:  # Use file to refer to the file object
        txt_query = f"INSERT INTO record_trades (stock_ticker, quantity_traded, side, trade_type, date, price, stock_sentiment) " \
                f"VALUES({stock_ticker}, {quantity}, {side}, {trade_type}, {now}, {limit_price}, {stock_sentiment})"
...
def wait_time(seconds=3):
    """
    This function pauses for the specified time, default to 3 seconds. Overwrites each print statement with progress.
...
def get_ma(api, stock_ticker, days=200):
    """
    This function returns the simple moving average in days of a stock based on historical data. The default is set to 200 days.

    Parameters
    ----------
    api: Alpaca api object
    stock_ticker: Stock ticker string (ex: NRZ)
    days: Number of days for which moving average is needed. Default is 200 days
    
    Returns
    -------
    moving_avg: The simple moving average of the stock for specified number of days

    """
    x = api.get_barset(stock_ticker,'day',days).df
    moving_avg = x[stock_ticker,"close"].mean()
    return moving_avg
    
    
def price(stream, stock_ticker):    
    """
    This function returns the last traded price of the stock in the IEX exchange using streaming connecting. Note that this value may not be accurate as data is limited to trades taking place in IEX.

    Parameters
    ----------
    stream: Alpaca stream object
    stock_ticker: Stock ticker string (ex: NRZ)
    
    Returns
    -------
    price: The price at which the stock was last traded in the market

    """

    async def quote_callback(q):
        print("quote", q)
    price = stream.subscribe_quotes(quote_callback, stock_ticker)
    stream.run()

def get_price(api, stock_ticker):    
    """
    This function returns the last traded price of the stock from the market.
    ...
    price = api.get_last_trade(stock_ticker)
    return price    
